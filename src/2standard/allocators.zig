...
// allows the programmer to choose exactly how memory allocations are done within
// the standard library - no allocations happen behind your back in the standard library.

// The most basic allocator is std.heap.page_allocator. Whenever this allocator makes
// an allocation it will ask your OS for entire pages of memory; an allocation of a single
// byte will likely reserve multiple kibibytes. As asking the OS for memory requires a system call this is also extremely inefficient for speed.
